...
// === GET MY PERFORMANCE
router.get('/task/detail/rating/performance', isAuthenticated, async (req, res) => {
    let user = req.authenticatedUser;
    // ambil data ratings
    let ratings = await knex('event_task_detail_ratings')
        .where('rated_for', '=', user.id);

    // cari rata2: jumlahkan semua rating lalu bagi dengan banyak rating
    let average = ratings.reduce((a, b) => b.rating + a, 0) / ratings.length;

    // Kalau belum punya rating, average akan berisikan NaN, jadi di override jadi 0 aja
    if (isNaN(average)) { average = 0; }

    // ambil data tugas event
    let events = await knex('event_task_details').where('user_id', '=', user.id);
    // ambil data createdby pada setiap event_task_details
    ratings = await Promise.all(ratings.map(async (e) => {
        let createdBy = await knex('users').where('id', '=', e.created_by).first();
        return { ...e, created_by: createdBy };
    }));

    let eventCount = events.length;
    // ambil jumlah task yang diikuti
    let eventActiveCount = events.filter(e => e.status == 1).length;
    // ambil jumlah task yang ditolak
    let eventRejectCount = events.filter(e => e.status == -1).length;
    // ambil jumlah task yang dikeluarkan
    let eventKickCount = events.filter(e => e.status == -3).length;

    return res.status(200).json({
        event_count: eventCount,
        event_active_count: eventActiveCount,
        event_reject_count: eventRejectCount,
        event_kick_count: eventKickCount,
        average: average,
        ratings: ratings
    });
});
